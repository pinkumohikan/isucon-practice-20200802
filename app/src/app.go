...
func recentHandler(w http.ResponseWriter, r *http.Request) {
	...
	if (totalCount - 100*page + 1) < 0 {
		rows, err = dbConn.Query("SELECT memo_id FROM public_memos BETWEEN ? AND ?", 0, (totalCount - memosPerPage*(page-1)))
	} else {
		rows, err = dbConn.Query("SELECT memo_id FROM public_memos BETWEEN ? AND ?", (totalCount - memosPerPage*page + 1), (totalCount - memosPerPage*(page-1)))
	}
	if err != nil {
		serverError(w, err)
		return
